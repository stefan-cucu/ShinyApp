                ...
                tabPanel(title = "V.A. Continue",
                         fluidRow(
                           column(
                             ...
                             splitLayout(
                               cellWidths = c("35%", "35%"),
                               numericInput("ex_2_cstart", "Start interval", value =
                                              0),
                               numericInput("ex_2_cend", "Sfarsit interval", value =
                                              0)
                             ),
...
server <- function(input, output, session) {
  ...
  output$ex2discrete <- renderUI({
    ...
    lapply(1:ex2_rvs$cnt, function(a) {
      ...
      box(
        title = sprintf("V.A. Discreta #%d", a),
        status = "primary",
        solidHeader = TRUE,
        collapsible = TRUE,
        "Functia de masa",
  ...
  output$ex2continue <- renderUI({
    ...
    lapply(1:ex2_fcts$cnt, function(a) {
      ...
      box(
        title = sprintf("V.A. Continua #%d", a),
        status = "warning",
        solidHeader = TRUE,
        collapsible = TRUE,
        "Functia de densitate",
  ...
  output$proprietati2 <- renderText({
    ...
    paste("Media repartitiei este:",
          EX,
          "\nVarianta repartitiei este:",
          VAR)
  })
  ...
  observeEvent(input$ex_2_btnc, {
    ...
    if (between(integrate(f, input$ex_2_cstart - 1, input$ex_2_cend)$value,
                0.95,
                1.05) == FALSE) {
      output$proprietatic2 <- renderText({
    ...
    output$proprietatic2 <- renderText({
      ...
      paste("Media repartitiei este: ",
            media,
            "\nDispersia repartitiei este: ",
            dispersia)
    })
    
    ex2_fcts$cnt <- ex2_fcts$cnt + 1
    ex2_fcts$arr[[ex2_fcts$cnt]] <-
      list(input$ex_2_f, input$ex_2_cstart, input$ex_2_cend)
  })
  #########
  # Ex. 5 #
  #########
  ...
  observeEvent(input$va_selected, {
    ...
    if (tip == "unif" ||
        tip == "norm" || tip == "exp" || startsWith(tip, "c")) {
  ...
  observeEvent(input$ex_5_btn, {
    ...
    if (tip == "geom") {
      X <-
        RV(0:input$ex_5_dim,
           dgeom(0:input$ex_5_dim, input$ex_1_geom_p))
    }
    
    if (tip == "hgeom") {
      X <-
        RV(
          0:input$ex_1_hgeom_k,
          dhyper(
            0:input$ex_1_hgeom_k,
            input$ex_1_hgeom_m,
            input$ex_1_hgeom_n,
    ...
    if (tip == "pois") {
      X <-
        RV(0:input$ex_5_dim,
           dpois(0:input$ex_5_dim, input$ex_1_pois_l))
    }
    ...
    observeEvent(input$ex_5_begin, {
      ...
      output$ex_5_tbl = renderDT(df, options = list(pageLength = 10))
    })
  })
  #########
  # Ex. 8 #
